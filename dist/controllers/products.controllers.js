...
export const getLatestProduct = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has("latest")) {
        product = await JSON.parse(myCache.get("latest"));
    }
    else {
...
export const getAllCategories = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has("categories")) {
        categories = await JSON.parse(myCache.get("categories"));
    }
    else {
...
export const getAdminProducts = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has("admin-product")) {
        product = await JSON.parse(myCache.get("admin-product"));
    }
    else {
...
export const getSingleProducts = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has(`single-product-${id}`)) {
        product = await JSON.parse(myCache.get(`single-product-${id}`));
    }
    else {
...
export const getAllSearchProducts = tryCatch(async (req, res, next) => {

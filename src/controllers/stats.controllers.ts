...
export const getDashboardStats = tryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (myCache.has("admin-stats")) {
      ...
      const reveneuePercentage = calculatePercentage(
        currentMonthRevenue,
        lastMonthRevenue
      );
      const count = {
      ...
      const categoriesCount: Record<string, number>[] = await getCategory({
        categories,
        productsCount,
      });
...
export const getBarCharts = tryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (myCache.has("admin-bar-charts")) {
      ...
      const productCounts = getChartData({ length: 6, docArr: products as [] });
      const usersCounts = getChartData({ length: 6, docArr: users as [] });
      const ordersCounts = getChartData({ length: 12, docArr: orders as [] });
...
export const getLineCharts = tryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (myCache.has("admin-line-charts")) {
      ...
      const [products, users, orders] = await Promise.all([
        Product.find(baseQuaery).select("createdAt"),
        User.find(baseQuaery).select("createdAt"),
        Order.find(baseQuaery).select(["createdAt", "discount", "total"]),
      ]);
